...
public class Tree : MonoBehaviour
{
    ...
    [Range(0,2)] public float startBranchLocationReducer = .9f;
...
*/
        ...
        foreach( Branch b in branches ){
            ...
            foreach( FlowerPoint p in b.flowers ){
                ...
                for( int j = 0;  j < 4; j ++ ){
                    ...
                    Vector3 left = Vector3.Cross( fNor , x ).normalized;

                    Vector3 newNor = Vector3.Cross(x , left).normalized;
    ...
    public void RebuildMeshFromData( float[] data_Verts , int[] data_Tris , float[] data_Flowers , int[] data_FlowersTris ){
        ...
        print( data_Verts.Length);
        print( data_Tris.Length);
        print( data_Flowers.Length);
        print( data_FlowersTris.Length);
...
*/
    ...
    public string GetDataPath(){
        ...
        string path = Application.dataPath + "/FantasyTree/Trees/" + saveName;
...
print( path);
    ...
    public List<float> FlattenParams(){
        ...
        paramList.Add((float)noiseOffset.x);
        paramList.Add((float)noiseOffset.y);
        paramList.Add((float)noiseOffset.z);
    ...
    public void UnflattenParams(float[] P ){
        ...
        noiseOffset             = new Vector3(    P[10],
                                                P[11],
                                                P[12]);
        minAngleMatch                           = P[13];
        minAngleReducer                         = P[14];
        maxAngleMatch                           = P[15];    
        maxAngleReducer                         = P[16];
        baseVsTipLength                         = P[17];
        baseVsTipLengthReducer                  = P[18];
        parentLengthMax                         = P[19];
        parentLengthMaxReducer                  = P[20];
        length                                  = P[21];
        lengthReducer                           = P[22];
        lengthVariation                         = P[23];
        lengthVariationReducer                  = P[24];
        upDesire                                = P[25];    
        upDesireReducer                         = P[26];
        startBranchLocation                     = P[27];
        startBranchLocationReducer              = P[28];
        endOfBranchWeight                       = P[29];
        endOfBranchWeightReducer                = P[30];
        width                                   = P[31];
        widthReducer                            = P[32];
        widthCutoff                             = P[33];
        numBarkColumns                          = (int)P[34];
        numBarkColumnsReducer                   = P[35];
        numBarkRows                             = (int)P[36];
        numBarkRowsReducer                      = P[37];
        numFlowers                              = (int)P[38];
        flowerSize                              = P[39];
        offsetSize                              = P[40];
